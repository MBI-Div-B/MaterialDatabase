# Guidelines for Contributing to material_database

The material_database repository uses nvie's branching model, known as 
[GitFlow][].
...
5. once the proposed code is considered ready by an appointed material_database 
...
- The code must comply with the material_database coding conventions, see below.
  [material_database travis-ci][] will check it for each Pull Request (PR) using
...
- The licensing terms for the contributed code must be compatible 
  with (and preferably the same as) the license chosen for the material_database 
